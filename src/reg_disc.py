...
def prepare_data(df):
    ...
    df["month0_risk"] = df["risk_scores"].apply(
        lambda scores: (scores[0] if isinstance(scores, list) and len(scores) > 0 else None)
    )
    ...
    df["treatment"] = df.apply(
        lambda row: (1 if row["intervention_applied"] and row["intervention_month"] == 0 else 0),
        axis=1,
    )
