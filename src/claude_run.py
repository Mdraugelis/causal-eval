...
def generate_population(
    ...
    print(f"Top {top_k} average true risk: {df['true_risk'].nlargest(top_k).mean():.4f}")
...
def simulate_monthly_intervention(df, intervention_efficacy=0.20, top_k=250, months=12):
    ...
    df.loc[treated_idx, "final_outcome"] = np.random.binomial(
        n=1, p=(df.loc[treated_idx, "true_risk"] * (1 - intervention_efficacy))
    )
    # Untreated patients use original risk
    untreated_idx = ~treated_idx
    df.loc[untreated_idx, "final_outcome"] = np.random.binomial(n=1, p=(df.loc[untreated_idx, "true_risk"]))
    ...
    print("Outcome simulation completed:")
...
def run_simulation_rd(
    ...
    try:
        sub_df, rd_model, rd_results = run_rd_analysis(
            df, monthly_thresholds, month=-1, bandwidth=bandwidth  # Last month
        )
