import unittest
import random
from src.intervention_sim import (
    RiskModel,
    Person,
    StrokeSimulationWithIntervention,
)
...
class TestRiskModel(unittest.TestCase):
    ...
    def test_invalid_parameters(self):
        ...
        with self.assertRaises(ValueError):
            self.risk_model.predict_risk(is_positive=False, neg_alpha=1.0, neg_beta=1.1)  # Should be < 1
...
class TestStrokeSimulationWithIntervention(unittest.TestCase):
    ...
    def test_intervention_limits(self):
        ...
        for month in range(3):  # Run only 3 months for testing
            ...
            eligible = [
                p for p in self.sim.population if p.is_alive and not p.has_stroke and not p.intervention_applied
            ]
            eligible_sorted = sorted(
                eligible,
                key=lambda p: current_risk_scores.get(p.person_id, 0),
                reverse=True,
            )
        ...
        for month in range(3):
            ...
            intervened_this_month = [
                p for p in self.sim.population if p.intervention_applied and p.intervention_month == month
            ]
            ...
            for p in self.sim.population:
                if (
                    not p.intervention_applied
                    and p.is_alive
                    and not p.has_stroke
                    and len(p.monthly_risk_scores) > month
                    and p.monthly_risk_scores[month] is not None
                    and p.monthly_risk_scores[month] > lowest_intervened_score
                ):
                    # This should never happen - higher risk people should get intervention first
                    self.fail(
                        "Person {} had higher risk ({}) than intervened person ({}) but didn't get "
                        "intervention".format(
                            p.person_id,
                            p.monthly_risk_scores[month],
                            lowest_intervened_score,
                        )
                    )
