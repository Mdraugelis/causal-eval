...
class TestClaudeRun(unittest.TestCase):
    ...
    def test_run_rd_analysis(self, mock_print):
        ...
        df.loc[df["treated"], "final_outcome"] = np.random.binomial(
            n=1, p=df.loc[df["treated"], "true_risk"] * 0.7
        )
        df.loc[~df["treated"], "final_outcome"] = np.random.binomial(
            n=1, p=df.loc[~df["treated"], "true_risk"]
        )
        
        # Thresholds list
        thresholds = [{"month": 0, "threshold": cutoff, "n_selected": df["treated"].sum()}]
    ...
    def test_run_simulation_rd(self, mock_plot, mock_rd, mock_simulate, mock_generate):
        ...
        mock_model.params = {"Intercept": 0.05, "rd_treat": -0.02, "dist": 0.1, "rd_treat:dist": 0.05}
